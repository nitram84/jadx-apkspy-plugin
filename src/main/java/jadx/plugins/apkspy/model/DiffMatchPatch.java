 ...
 * Reformatted by Lucas Baizer (developer of apkSpy).
 */
...
public class DiffMatchPatch {
	 ...
	 * very loose). Note that MatchThreshold controls how closely the end points of
	 ...
	 */
	private short marginMaxBits = 32;

	/**
	 * Internal class for returning results from diffLinesToChars(). Other less
	 ...
	 * diff. This method allows the 'checklines' of diffMain() to be optional. Most
			...
			switch (thisDiff.operation) {
				...
				case EQUAL:
					...
					if (countDelete >= 1 && countInsert >= 1) {
						...
						for (Diff subDiff : diffMain(textDelete, textInsert, false, deadline)) {
							pointer.add(subDiff);
						}
					}
					countInsert = 0;
	...
	public void diffCleanupEfficiency(LinkedList<Diff> diffs) {
		...
		while (thisDiff != null) {
			if (thisDiff.operation == Operation.EQUAL) {
				// Equality found.
				if (thisDiff.text.length() < diffEditCost && (postInx || postDel)) {
					// Candidate found.
					equalities.push(thisDiff);
					preIns = postInx;
					preDel = postDel;
				 ...
				 */
				if (lastEquality != null && ((preIns && preDel && postInx && postDel)
						|| ((lastEquality.length() < diffEditCost / 2) && ((preIns ? 1 : 0) + (preDel ? 1 : 0)
								+ (postInx ? 1 : 0) + (postDel ? 1 : 0)) == 3))) {
					// System.out.println("Splitting: '" + lastEquality + "'");
					// Walk back to offending equality.
					while (thisDiff != equalities.peek()) {
						thisDiff = pointer.previous();
		...
		for (String token : tokens) {
			...
			switch (token.charAt(0)) {
				...
				case '=':
					...
					String text;
					try {
						text = text1.substring(pointer, pointer + n); // TODO
						pointer += n;
	 ...
	 */
	public int matchBitmap(String text, String pattern, int loc) {
		// Check for null inputs.
		if (text == null || pattern == null) {
			throw new IllegalArgumentException("Null inputs. (matchBitmap)");
		}
	...
	protected int matchBitap(String text, String pattern, int loc) {
		assert (marginMaxBits == 0 || pattern.length() <= marginMaxBits) : "Pattern too long for this application.";
	...
	protected void patchAddContext(Patch patch, String text) {
		...
		while (text.indexOf(pattern) != text.lastIndexOf(pattern)
				&& pattern.length() < marginMaxBits - patchMargin - patchMargin) {
			padding += patchMargin;
	...
	public Object[] patchApply(LinkedList<Patch> patches, String text) {
		...
		for (Patch aPatch : patches) {
			...
			int startLoc;
			int endLoc = -1;
			if (text1.length() > this.marginMaxBits) {
				// patchSplitMax will only provide an oversized pattern in the case of
				// a monster delete.
				startLoc = matchBitmap(text, text1.substring(0, this.marginMaxBits), expectedLoc);
				if (startLoc != -1) {
					endLoc = matchBitmap(text, text1.substring(text1.length() - this.marginMaxBits),
							expectedLoc + text1.length() - this.marginMaxBits);
				...
				startLoc = matchBitmap(text, text1, expectedLoc);
			}
			if (startLoc == -1) {
				// No match found. :(
				results[x] = false;
				...
				if (endLoc == -1) {
					...
					text2 = text.substring(startLoc, Math.min(endLoc + this.marginMaxBits, text.length()));
				}
				if (text1.equals(text2)) {
	...
	public void patchSplitMax(LinkedList<Patch> patches) {
		...
		while (bigpatch != null) {
			if (bigpatch.length1 <= marginMaxBits) {
